...
enum StateID
{
	...
	ID_C2_HA_ONLY,
	ID_C3_DP_ONLY,
	ID_D1_NOK9LeftBehind,
	ID_D2_PressOKToConfirm,
	ID_D6_NoK9LeftBehindPowerDownByDoorOpened,
	ID_D7_PowerDownByOKPress,
	ID_D8_PowerDownByIgnitionOFF,
	ID_D9_PowerDownByHAandDPSetToAlwaysOFF,
	ID_D10_PowerDownByPowerPress,
	ID_D11_UpdatingFirmware,
	ID_E1_VIMCommunicationsError,
	ID_E3_UNKNOWN,
	ID_S1_Sleep,
	ID_G1_SystemTestConfirm,
	ID_G2_SystemTest
};
...
void Init_Timers()
{
	...
	// CheckCellSignal_Timer
	CheckCellSignal_Timer.Name = "CheckCellSignal_Timer";
	CheckCellSignal_Timer.OverFlowFlag = false;
	CheckCellSignal_Timer.Threshold = 300000; // 5min = 300 s = 300,000 ms
	CheckCellSignal_Timer.TimerEnable = false;
}
...
void Update_Timers()
{
	...
	CheckCellSignal_Timer.CheckOverflow();
}
...
void Process_System_Alarm_States()
{
	...
	else
	{
		...
		if (Current_System_Alarm_State == AlarmState::a_PreAlarm)
		{
			...
			{
				// Battery Flag
				if (data_global.batt_error_previous == true)
				{
					data_global.battchanged = true;
				}
				// Stall Flag
				if (data_global.engineStalled_previous == true)
				{
					data_global.engine_changed = true;
				}
				// Temperature Flag
				if (data_global.temp_alarmFlag_previous == true)
				{
					data_global.temp_changed = true;
				}
				// Temperature Sensor Flag
				if (data_global.temp_errorFlag_previous == true)
				{
					data_global.temp_changed = true;
				}
				// DaisyChain Flag
				if (data_global.Aux1Input_previous == true || data_global.Aux2Input_previous == true)
				{
			...
			else if (SystemAlarm_Timer.OverFlowFlag == true)
			{
				...
				// Check Stall
				if (data_global.engineStalled_current == true)
				{
					data_global.engine_changed = true;
				}
				// Temperature Flag
				if (data_global.temp_alarmFlag_current == true)
				{
					data_global.temp_changed = true;
				}
				...
				// DiasyChain Flag
				if (data_global.Aux1Input_current == true || data_global.Aux2Input_current == true)
				{
					data_global.aux_changed = true;
				}
			...
			if (CurrentPreAlarmCounter != PreviousPreAlarmCounter)
			{
				...
				if (DisplayPreAlarmCounter < 0)
				{
					DisplayPreAlarmCounter = 0;
				}
		...
		else if (Current_System_Alarm_State == AlarmState::a_Snooze)
		{
			if (Previous_System_Alarm_State != AlarmState::a_Snooze)
			{
				...
				if (CurrentSnoozeAlarmCounter != PreviousSnoozeAlarmCounter)
				{
					...
					// Make sure it doesn't go below zero
					if (DisplaySnoozeAlarmCounter < 0)
					{
						DisplaySnoozeAlarmCounter = 0;
					}
		...
		else if (Current_System_Alarm_State == AlarmState::a_None)
		{
			...
			if (data_global.batt_error_current == true || data_global.engineStalled_current == true || data_global.temp_alarmFlag_current == true || data_global.temp_errorFlag_current == true || data_global.Aux1Input_current == true || data_global.Aux2Input_current == true)
			{
				...
				if (systemsettings_current.InitialPowerUpFlag == true && systemsettings_default.AutoSnoozeEnabled == true)
				{
...
void check_cell_signal()
{
	...
	if (CheckCellSignal_Timer.OverFlowFlag == true)
	{
		CheckCellSignal_Timer.OverFlowFlag = false;
		CheckCellSignal_Timer.SyncTimerVal();
	...
	if (CurrentFOTACode != FOTA_Done)
	{
		...
		case FOTA_CheckFW:
			...
			else
			{
				...
				if (Check_ACK())
				{
					MONITOR.println("FOTA CheckFW:CheckACK");
					// Move on to CheckFW
